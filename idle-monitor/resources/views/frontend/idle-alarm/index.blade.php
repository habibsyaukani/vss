...
@section('title', 'Idle Monitor')

{{-- Cache Buster: v2.1 - FIXED Tree Children Display - 2026-06-11 18:00 --}}
                        ...
                        <ul class="tree-children">
                            @foreach($groupData['devices'] as $device)
                                <li class="tree-child" data-location="{{ $device->location ?? '' }}" data-series="{{ $device->series ?? '' }}">
                                    ...
                                    @endphp
                                    <i class="fas {{ $deviceIcon }} group-icon me-2" style="color: {{ $iconColor }}; font-size: 11px;"></i>
...
$(document).ready(function() {
    ...
    function triggerExport(data) {
        ...
        $.ajax({
            url: "{{ route('frontend.idle-alarm.export') }}",
            type: 'POST',
            ...
            success: function(res) {
                if (res.use_queue) {
                    ...
                    let pollInterval = setInterval(function() {
                        $.get("{{ url('/idle-alarm/export-status') }}/" + res.job_id, function(statusRes) {
                    ...
                    // Direct download
                    $('#exportLoading').hide();
                    let form = $('<form>', { action: "{{ route('frontend.idle-alarm.export') }}", method: 'POST' })
