...
echo "===========================================\n";
echo "  IMPORT DEVICES - CORRECT VERSION\n";
echo "===========================================\n\n";
...
while (($data = fgetcsv($handle)) !== false) {
    $lineNumber++;
    $devices[] = [
        'device_code' => trim($data[0]),  // From CSV
...
try {
    ...
    foreach ($devices as $device) {
        try {
            // Map CSV columns to correct DB columns
            DB::table('devices')->insert([
                'device_id' => $device['unit_code'],  // device_id = unit_code
                'device_name' => $device['device_code'],  // device_name = device_code (GPE-DT-xxx)
